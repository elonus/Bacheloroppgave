...
x.vals = seq(from = 10, to = 12.5, by = 0.1)
...
predict.int.survmod <- data.frame(lower = predict.survmod$link - qt(0.975, df = nrow(anthuneq)))
predict.int.survmod$upper <- predict.survmod$link + qt(0.975, df = nrow(anthuneq))
predict.int.survmod$x.vals <- x.vals
predict.int.survmod$fit <- predict.survmod$link
...
uneq.surv.plot <- ggplot() +
  geom_point(data = anthuneq, aes(finger, height), size = size) +
  geom_abline(slope = survcoef[2], intercept = survcoef[1], color = "blue") +
  geom_point(data = predict.int.survmod, aes(x = x.vals, y = lower), color = "yellow") +
  geom_point(data = predict.int.survmod, aes(x = x.vals, y = upper), color = "green")

uneq.normal.plot <- ggplot() +
  geom_point(data = anthuneq, aes(finger, height), size = size) +
  geom_abline(slope = normalcoef[2], intercept = normalcoef[1], color = "red") +
  geom_point(data = predict.int.normalmod, aes(x = x.vals, y = lwr), color = "yellow") +
  geom_point(data = predict.int.normalmod, aes(x = x.vals, y = upr), color = "green")

srs.surv.plot <- ggplot() +
  geom_point(data = anthsrs, aes(finger, height)) +
  geom_abline(slope = survcoef[2], intercept = survcoef[1], color = "blue") +
  geom_point(data = predict.int.survmod, aes(x = x.vals, y = lower), color = "yellow") +
  geom_point(data = predict.int.survmod, aes(x = x.vals, y = upper), color = "green")

srs.normal.plot <- ggplot() +
  geom_point(data = anthsrs, aes(finger, height)) +
  geom_abline(slope = normalcoef[2], intercept = normalcoef[1], color = "red") +
  geom_point(data = predict.int.normalmod, aes(x = x.vals, y = lwr), color = "yellow") +
  geom_point(data = predict.int.normalmod, aes(x = x.vals, y = upr), color = "green")
