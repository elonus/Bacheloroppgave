...
api.srs.clus <- apipop[sample(1:nrow(apipop), nrow(apiclus1), replace = F),]
api.srs.clus$pw <- nrow(apipop) / nrow(apiclus1)
api.srs.clus$fpc <- nrow(apipop)
...
api.srs.90 <- apipop[sample(1:nrow(apipop), 800, replace = F),]
api.srs.90$pw <- nrow(apipop) / 800
api.srs.90$fpc <- nrow(apipop)

svy.api.srs.90 <- svydesign(id = ~1, weights = ~pw, fpc = ~fpc, data = api.srs.90)

api.srs.90.svymod <- svyglm(api00 ~ avg.ed, design = svy.api.srs.90)
```
...
srs.90.svy.cov <- vcov(api.srs.90.svymod)
predict.api.srs.90.svymod <- as.data.frame(predict(api.srs.90.svymod, newdata = data.frame(avg.ed = x.vals), se=TRUE))
predict.int.api.srs.90.svymod <- data.frame(fit = predict.api.srs.90.svymod$link)
predict.int.api.srs.90.svymod$x.vals <- x.vals
predict.int.api.srs.90.svymod$se <- sqrt(var(api.srs.90.svymod$residuals) + srs.90.svy.cov[1, 1] + predict.int.api.srs.90.svymod$x.vals^2 * srs.90.svy.cov[2, 2] + 2*predict.int.api.srs.90.svymod$x.vals * srs.90.svy.cov[1, 2])
predict.int.api.srs.90.svymod$lower <- predict.int.api.srs.90.svymod$fit - predict.int.api.srs.90.svymod$se * qt(0.975, df = nrow(apipop))
predict.int.api.srs.90.svymod$upper <- predict.int.api.srs.90.svymod$fit + predict.int.api.srs.90.svymod$se * qt(0.975, df = nrow(apipop))
```
...
percent.in.pred.val(apipop$avg.ed, apipop$api00, predict.int.api.srs.90.svymod$x.vals, predict.int.api.srs.90.svymod$lower, predict.int.api.srs.90.svymod$upper)
...
base <- ggplot() + xlim(1, 5) + ylim(346, 969) +
  theme(axis.title = element_text(size = 20), axis.text = element_text(size = 15)) +
  ylab("Height (cm)") + xlab("Finger length (cm)")
...
```{r}
api.clus.svymod.coef <- coef(api.clus.svymod)
api.srs.90.svymod.coef <- coef(api.srs.90.svymod)
base +
  geom_point(data = apipop, aes(avg.ed, api00)) +
  geom_ribbon(aes(x = x.vals, ymin = lower, ymax = upper), data = predict.int.api.srs.90.svymod, fill = "blue", alpha = 0.5) +
  geom_ribbon(aes(x = x.vals, ymin = lower, ymax = upper), data = predict.int.api.clus.svymod, fill = "red", alpha = 0.5) +
  geom_abline(slope = api.srs.90.svymod.coef[2], intercept = api.srs.90.svymod.coef[1], color = "blue", lwd = 1) +
  geom_line(data = predict.int.api.srs.90.svymod, aes(x = x.vals, y = lower), color = "blue", linetype = "dashed") +
  geom_line(data = predict.int.api.srs.90.svymod, aes(x = x.vals, y = upper), color = "blue", linetype = "dashed") +
  geom_abline(slope = api.clus.svymod.coef[2], intercept = api.clus.svymod.coef[1], color = "red", lwd = 1) +
  geom_line(data = predict.int.api.clus.svymod, aes(x = x.vals, y = lower), color = "red", linetype = "dashed") +
  geom_line(data = predict.int.api.clus.svymod, aes(x = x.vals, y = upper), color = "red", linetype = "dashed")
```
