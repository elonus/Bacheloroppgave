...
svy.api.strat <- svydesign(id=~1,strata=~stype, weights=~pw, data=apistrat, fpc=~fpc)
api.strat.svymod <- svyglm(api00 ~ avg.ed, design = svy.api.strat)
api.strat.normmod <- lm(api00 ~ col.grad, data = apistrat)
summary(api.strat.svymod)
summary(api.strat.normmod)
```

```{r}
modE <- svyglm(api00 ~ avg.ed, design = subset(svy.api.strat, stype == "E"))
modM <- svyglm(api00 ~ avg.ed, design = subset(svy.api.strat, stype == "M"))
modH <- svyglm(api00 ~ avg.ed, design = subset(svy.api.strat, stype == "H"))
...
predict.int.api.clus.srs.svymod$lower <- predict.int.api.clus.srs.svymod$fit - predict.int.api.clus.srs.svymod$se * qt(0.975, df = nrow(apiclus1))
predict.int.api.clus.srs.svymod$upper <- predict.int.api.clus.srs.svymod$fit + predict.int.api.clus.srs.svymod$se * qt(0.975, df = nrow(apiclus1))
